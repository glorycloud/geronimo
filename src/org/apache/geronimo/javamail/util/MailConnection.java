...
public class MailConnection {
	...
	Logger log = LoggerFactory.getLogger(MailConnection.class);
    /**
    ...
    protected static int objCounter = 1; //debug purpose only
    protected int objId = 0; //debug purpose only
    ...
    protected MailConnection(ProtocolProperties props) 
    {
        ...
        objId = objCounter++;
		log.debug( "Connection object created: ID: {}" ,objId );
		//showCallStack();
    }
    ...
    public boolean protocolConnect(String host, int port, String username, String password) throws MessagingException {
    	log.debug( "Connection connected. ID:{}" ,objId);
    ...
    protected void getConnection() throws IOException, MessagingException
    {
        ...
        if (socket == null) {
            ...
            if (sslConnection) {
                ...
                //socket.setSoTimeout(30000);//time out in 30 seconds
            }
            else
            {
                getConnectedSocket();
    ...
    protected List selectSaslMechanisms() {
        ...
        List supported = null;//getServerMechanisms(); 
