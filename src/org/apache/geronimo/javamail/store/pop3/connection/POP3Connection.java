 ...
 *
 * @version $Rev: 597135 $ $Date: 2007-11-22 00:26:57 +0800 () $
 */
public class POP3Connection extends MailConnection implements POP3Constants {
    ...
    public synchronized POP3Response sendCommand(String cmd, boolean multiLine) throws MessagingException {
    ...
    private String readLine() throws MessagingException
    {
        ...
        try {
            ...
            while ((byteRead = inputStream.read()) > 0) {
            	...
            	{
            		byte[] b = lineBuf.toByteArray();
            		return new String(b, 0, b.length -1);
            	}
            	lineBuf.write(byteRead);
            	lastByte = byteRead;
            }
        } catch (IOException e) {
            throw new MessagingException("Error processing a read line response", e);
        }
    ...
    private final byte[] COMMANDS_TERM = new byte[] { (byte)'\r', (byte)'\n',  (byte)'.', (byte)'\r', (byte)'\n'};
    /**
...
//        }
        ...
        CircularFifoBuffer termBuf = new CircularFifoBuffer(5);
        ...
        readingNetwork:
            while ((charsRead = inputStream.read(buffer)) > 0) {
                ...
                int tailCnt = Math.min(5,  charsRead);
                for(int i=tailCnt;i>0;i--)
                {
                	termBuf.add(buffer[charsRead-i]);
                }
                if(termBuf.isFull())
                {
                	Iterator<Byte> it = termBuf.iterator();
                	int i=0;
                	while(it.hasNext())
                	{
                		if(it.next().byteValue() != COMMANDS_TERM[i])
                			continue readingNetwork;//not terminal, continue
                		i++;
                	}
                	break readingNetwork;
                }
            }
            out.flush(); 
        } catch (IOException e) {
    ...
    /**
     * Tim Liu add this overloading function to retrieve specified lines of message, not all
     * of the message data.
     * @param sequenceNumber
     * 					The message sequence number.
     * @param lines
     ...
     */
    public byte[] retrieveMessageData(int sequenceNumber,int lines) throws MessagingException {
     //   POP3Response msgResponse = sendMultiLineCommand("RETR " + sequenceNumber); 
        POP3Response msgResponse = sendMultiLineCommand("TOP " + sequenceNumber + " "+lines); 
        return msgResponse.getData();          
    }
