...
import org.apache.geronimo.javamail.store.pop3.connection.POP3Response;
 ...
 * 
 * @version $Rev: 689140 $ $Date: 2008-08-27 01:20:01 +0800 (周三, 27 八月 2008) $
 */
public class POP3Folder extends Folder {
    ...
    public void open(int mode) throws MessagingException {
        ...
        try {
            ...
            uidCache = new String[msgCount];
            //POP3Connection conn = getConnection();
            
            if(msgCount > 0)
            {
	            POP3Response rsp = connection.sendCommand("UIDL", true);
	            
	            InputStreamReader isr = new InputStreamReader(rsp.getContentStream());
	            BufferedReader bufReader = new BufferedReader(isr);
	            String s;
	            int idx = 0;
	            while(( s = bufReader.readLine()) != null && idx < msgCount)
	            {
	            	int pos = s.indexOf(' ');
	            	if(pos < 1)//not a valid uid item,such as in wo.com.cn, the first line is "+OK".
	            		continue;	            		
	            	uidCache[idx++] = s.substring(pos+1);
	            }
            }
        } catch (Exception e) {
    ...
    public Message getMessage(int msgNum) throws MessagingException {
        ...
        if (msg == null) {
            msg = new POP3Message(this, msgNum, uidCache[msgNum - 1]); 
    ...
    public void refreshMessageCount() throws MessagingException
    {
		POP3Connection conn = null;
		try
		{
			conn = ((POP3Store)store).createNewConnection();
			POP3StatusResponse rsp = conn.retrieveMailboxStatus();
			this.msgCount = rsp.getNumMessages();
