 ...
 * content
 * 
 * @version $Rev: 597135 $ $Date: 2007-11-22 00:26:57 +0800 (007) $
 */
public class POP3Message extends MimeMessage {
    ...
    // Tim Liu add this filed for storing the status whether this message 
	// has been partly loaded.
	protected boolean partialLoadedFlag = false;
	//this member is stored for checking whether this time we only need to load
	//partial content in function loadContent.
	protected boolean toLoadPartialFlag = false;
	protected boolean hasAttachmentFlag = false;
	protected boolean attahcmentReadFlag = false;
	protected LinkedList<String> attachNames = new LinkedList<String>();
	//max mail size for loading partial content.
	protected static int maxMailSizeForPartialLoading = 32768;
    ...
    protected POP3Message(Folder folder, int msgnum, String uid) {
        this(folder, msgnum);
        this.uid = uid;
    }
  ...
  ///Added by Tim Liu
	public int getMaxSizeForPartialLoading()
	{
		return maxMailSizeForPartialLoading;
	}
	public void setMaxSizeForPartialLoading(int maxSize)
	{
		maxMailSizeForPartialLoading = maxSize;
	}
	protected boolean recursivelyCheckAttachment(Part p) throws Exception
	{
...
//		}
		...
		if(p.isMimeType("multipart/*"))
		{
			...
			{
				if(recursivelyCheckAttachment(mp.getBodyPart(i)))
					return true;
			}
		}
		else
	...
	//			System.out.println("has attachment " + filename + " , size:"
	//								+ p.getSize());
	...
	//			((MimeBodyPart)p).saveFile(f);
			...
			{
//				System.out.println("text part:" + (String) p.getContent());
				return false;
			}
		}
		return false;
	...
	/**
	 * Tim Liu add this function for checking attachment without downloading
	 * whole contents of mail.
	 */
	public boolean hasAttachment() {	
		if(attahcmentReadFlag)
			return hasAttachmentFlag;
		try {
			// recursively check the part of message.
			toLoadPartialFlag = true;
			//loadContent function will be called automatically during Mulipart.getCount().
			hasAttachmentFlag = recursivelyCheckAttachment(this);
			
			this.attahcmentReadFlag = true;
		} catch (Exception expt) {
			expt.printStackTrace();
		}
		finally
		{
			toLoadPartialFlag = false;//restore this flag.
		}
		return hasAttachmentFlag;
	}
	///end by Tim Liu
	...
	/**
	 * Tim Liu add this function 05/28/2012, return attachment's name list
	 * if there's no attachment, empty list will be returned.
	 */
	 ...
	 * this function can be invoked automatically during Multipart.getContent().
	 * @exception MessagingException
	 */
	protected void loadContent() throws MessagingException {
		if (content == null || partialLoadedFlag == true) {
			boolean tmpPartialLoadedFlag = partialLoadedFlag;
			// retrieve (and save the raw message data
//			messageData = connection.retrieveMessageData(msgnum);
			if (this.getSize() > maxMailSizeForPartialLoading) 
			{
				POP3Connection connection = getConnection();//getSize may create POP3connection also,  we should get connection after getSize() to avoid multi connection instance
				try {
					if(partialLoadedFlag && toLoadPartialFlag)
						return;
					else if(!partialLoadedFlag && toLoadPartialFlag)
					{
						messageData = connection.retrieveMessageData(msgnum,
							maxMailSizeForPartialLoading / 76);
						tmpPartialLoadedFlag = true;
					}
					else
					{
						messageData = connection.retrieveMessageData(msgnum);
						tmpPartialLoadedFlag = false;
					}
				} finally {
			...
			// now create a input stream for splitting this into headers and content
			...
			loadContent(in);
			partialLoadedFlag = tmpPartialLoadedFlag;
		}
	}
