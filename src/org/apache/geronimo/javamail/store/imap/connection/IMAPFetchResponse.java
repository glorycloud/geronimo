...
public class IMAPFetchResponse extends IMAPUntaggedResponse {
    ...
    protected List<IMAPFetchDataItem> dataItems = new ArrayList<IMAPFetchDataItem> ();
            ...
            // not, 
                ...
                switch (section.section) {
                    case IMAPBodySection.BODY:
                        ...
                        IMAPBody imapbody = new IMAPBody(section, source.readByteArray()); 
                        byte[] buf = imapbody.getContent();
                        ByteArrayInputStream in = new ByteArrayInputStream(buf);
                        int stremCount = in.available();
                        IMAPInternetHeader headersItem = new IMAPInternetHeader(section,in);
                        Enumeration en = headersItem.headers.getAllHeaders();
                        boolean allAreContent = true;
                        if(en.hasMoreElements())
                        {//IMAP may return only content and no headers on FETCH. Then whole line will be treaded as header line
                         //but his header line will be invalid and has no value
                        	Header  h = (Header) en.nextElement();
                        	if(h.getName().length() < 200 && h.getValue().length() > 0)
                        	{
                        		dataItems.add(headersItem);
                        		allAreContent = false;
                        	}
                        }
                        
                        //header read the stream until encounter blank line, some command "fetch 691 (BODY.PEEK[1.2])" 
                        //return string without any blank line, so such contents are all considered as header, whereas it
                        //should be body text.
                        byte[] text = null;
                        if(allAreContent || in.available() == 0)
                        	text = Arrays.copyOfRange(buf,buf.length - stremCount, buf.length);
                        else
                        	text = Arrays.copyOfRange(buf,buf.length - in.available(), buf.length);
                 //       byte[] text = Arrays.copyOfRange(buf,buf.length - in.available(), buf.length);
                        dataItems.add(new IMAPMessageText(section, text)); 
                        break; 
