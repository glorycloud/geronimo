...
//    private boolean useRawSearchResponse = false;
//    
//    public void setUseRawSearchResonse(boolean useRaw)
//    {
//    	this.useRawSearchResponse = useRaw;
//    }
    public IMAPResponseStream(InputStream in) {
        this.in = new PushbackInputStream(in, 2048);
...
//            	if(useRawSearchResponse)
//            		return new IMAPSearchRawResponse(data, tokenizer); 
        ...
        {
        	token = tokenizer.next();
        	return new IMAPUntaggedResponse(token.getValue(), data);
        }
    ...
    public void readBuffer() throws MessagingException {
    	try {
    		int minRequiredBytes = 1;
readmore:	while (true) {
	        	byte[] buf = new byte[Math.max(minRequiredBytes, Math.min(in.available(), 1024))];
	        	int buflen = in.read(buf);
	        	if(buflen == -1)
	        		throw new MessagingException("Unexpected error on server stream");
	        	...
	        	for(i=0;i<buflen;i++)
	        	{
	        		if(buf[i] == '\r')
	        		{
	        			...
	        			{// \r is the end
	        				in.unread(buf[i]);
	        				minRequiredBytes = 2; //so we will not read this '\r' infinitely 
	        				continue readmore;
	        			}
	        			else
	        			{
	        				...
	        				{
	        					if(i + 2 < buflen)
	        						in.unread(buf, i + 2, buflen -i -2); //skip the CRLF
	        					checkLiteral();
	        					return;
	        				}
	        				else
