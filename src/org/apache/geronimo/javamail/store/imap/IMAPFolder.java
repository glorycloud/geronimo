...
/**
 * The base IMAP implementation of the javax.mail.Folder This is a base class
 * for both the Root IMAP server and each IMAP group folder.
 * 
...
{
//	public static interface IdleHook
//	{
//		void acquire();
//		void release();
//		boolean hasDoneSend();
//	}
	...
	public static interface IdleHook extends Runnable
	{}
	static Logger log = LoggerFactory.getLogger(IMAPFolder.class);

	/**
	 * Special profile item used for fetching SIZE and HEADER information. These
	 * items are extensions that Sun has added to their IMAPFolder
	 * immplementation. We're supporting the same set.
	 */
	...
	protected Map<Long, IMAPMessage> uidToMsg = new HashMap<Long, IMAPMessage>();

	protected Map<Integer, Long> seqToUid = new HashMap<Integer, Long>();
	protected Map<Long, Integer> uidToSeq = new HashMap<Long, Integer>();
	// the separator the server indicates is used as the hierarchy separator
	protected char separator;
	// the "full" name of the folder. This is the fully qualified path name for
	// the folder returned by
	// the IMAP server. Elements of the hierarchy are delimited by "separator"
	// characters.
	protected String fullname;
	// the name of this folder. The is the last element of the fully qualified
	// name.
	...
	// the reported count of new messages (updated as a result of untagged
	// message resposes)
	...
	// Our cached status information. We will only hold this for the timeout
	// interval.
	protected IMAPMailboxStatus cachedStatus;
	// Folder information retrieved from the server. Good info here indicates
	// the
	// folder exists.
	...
	// Our current connection. We get one of these when opened, and release it
	// when closed.
	// We do this because for any folder (and message) operations, the folder
	// must be selected on
	// the connection.
	// Note, however, that there are operations which will require us to borrow
	// a connection
	// temporarily because we need to touch the server when the folder is not
	// open. In those
	// cases, we grab a connection, then immediately return it to the pool.
	private IMAPConnection currentConnection;
	 ...
	 * @param separator
	 *            The separtor character used to delimit the different levels of
	 *            the folder hierarchy. This is used to decompose the full name
	 *            into smaller parts and create the names of subfolders.
	 */
	...
	protected IMAPFolder(IMAPStore store, IMAPListResponse listInfo)
	{
		super(store);
		this.session = store.getSession();
		this.fullname = listInfo.mailboxName;
	...
	/**
	 * Retrieve the folder name. This is the simple folder name at the its
	 * hiearchy level. This can be invoked when the folder is closed.
	 * 
	 * @return The folder's name.
	 */
	public String getName()
	{
		// At the time we create the folder, we might not know the separator
		// character yet.
	...
	/**
	 * Retrieve the folder's full name (including hierarchy information). This
	 * can be invoked when the folder is closed.
	 * 
	...
	/**
	 * Return the parent for this folder; if the folder is at the root of a
	 * heirarchy this returns null. This can be invoked when the folder is
	 * closed.
	 * 
	...
	public Folder getParent() throws MessagingException
	{
		...
		// we don't hold a reference to the parent folder, as that would pin the
		// instance in memory
	...
	/**
	 * Check to see if this folder physically exists in the store. This can be
	 * invoked when the folder is closed.
	 * 
	...
	/**
	 * Internal routine for checking existance using an already obtained
	 * connection. Used for situations where the list information needs updating
	 * but we'd end up acquiring a new connection because the folder isn't open
	 * yet.
	 * 
	 * @param connection
	...
	/**
	 * Return a list of folders from this Folder's namespace that match the
	 * supplied pattern. Patterns may contain the following wildcards:
	 * <ul>
	 * <li>'%' which matches any characater except hierarchy delimiters</li>
	 ...
	 * 
	 * @return a possibly empty array containing Folders that matched the
	 *         pattern
	 * @throws MessagingException
	 *             if there was a problem accessing the store
	 */
	...
	{
		// go filter the folders based on the pattern. The server does most of
		// the
	 ...
	 * the supplied pattern. If the store does not support the concept of
	 * subscription then this should match against all folders; the default
	 * implementation of this method achieves this by defaulting to the
	 * {@link #list(String)} method.
	 * 
	 * @param pattern
	 *            the pattern to search for
	 * 
	 * @return a possibly empty array containing subscribed Folders that matched
	 *         the pattern
	 * @throws MessagingException
	 *             if there was a problem accessing the store
	 */
	 ...
	 * 
	 * @return The combination of Folder.HOLDS_MESSAGES and
	 *         Folder.HOLDS_FOLDERS, depending on the folder capabilities.
	 * @exception MessagingException
	 */
	 ...
	 * name will be recursively created. If the folder was sucessfully created,
	 * a {@link FolderEvent#CREATED CREATED FolderEvent} is sent to all
	 * FolderListeners registered with this Folder or with the Store.
	 * 
	 * @param newType
	 *            the type, indicating if this folder should contain subfolders,
	 *            messages or both
	 * 
	...
	public synchronized boolean create(int newType) throws MessagingException
	{
		...
		try
		{
			...
			// if this folder is expected to only hold additional folders, we
			// need to
	...
	public synchronized boolean isSubscribed()
	{
		try
		{
			...
			try
			{
				...
				{
					// a NOSELECT flag response indicates the mailbox is no
					// longer
	...
	/**
	 * Check to see if this Folder conatins messages with the
	 * {@link Flag.RECENT} flag set. This can be used when the folder is closed
	 * to perform a light-weight check for new mail; to perform an incremental
	 * check for new mail the folder must be opened.
	 * 
	...
	public Folder getFolder(String name) throws MessagingException
	{
		...
		// our separator does not get determined until we ping the server for
		// it. We
		// might need to do that now, so we need to use the getSeparator()
	...
	/**
	 * Delete this folder and possibly any subfolders. This operation can only
	 * be performed on a closed folder. If recurse is true, then all subfolders
	 * are deleted first, then any messages in this folder are removed and it is
	 * finally deleted; {@link FolderEvent#DELETED} events are sent as
	 * appropriate. If recurse is false, then the behaviour depends on the
	 * folder type and store implementation as followd:
	 * <ul>
	 * <li>If the folder can only conrain messages, then all messages are
	 * removed and then the folder is deleted; a {@link FolderEvent#DELETED}
	 * event is sent.</li>
	 * <li>If the folder can onlu contain subfolders, then if it is empty it
	 * will be deleted and a {@link FolderEvent#DELETED} event is sent; if the
	 * folder is not empty then the delete fails and this method returns false.</li>
	 * <li>If the folder can contain both subfolders and messages, then if the
	 * folder does not contain any subfolders, any messages are deleted, the
	 * folder itself is deleted and a {@link FolderEvent#DELETED} event is sent;
	 * if the folder does contain subfolders then the implementation may choose
	 * from the following three behaviors:
	 * <ol>
	 * <li>it may return false indicting the operation failed</li>
	 * <li>it may remove all messages within the folder, send a
	 * {@link FolderEvent#DELETED} event, and then return true to indicate the
	 * delete was performed. Note this does not delete the folder itself and the
	 * {@link #exists()} operation for this folder will return true</li>
	 * <li>it may remove all messages within the folder as per the previous
	 * option; in addition it may change the type of the Folder to only
	 * HOLDS_FOLDERS indictaing that messages may no longer be added</li></li>
	 * </ul>
	...
	/**
	 * Rename this folder; the folder must be closed. If the rename is
	 * successfull, a {@link FolderEvent#RENAMED} event is sent to all listeners
	 * registered with this folder or with the store.
	 * 
	 * @param newName
	...
	/**
	 * Open this folder; the folder must be able to contain messages and must
	 * currently be closed. If the folder is opened successfully then a
	 * {@link ConnectionEvent#OPENED} event is sent to listeners registered with
	 * this Folder.
	 * <p/>
	...
	public synchronized void open(int mode) throws MessagingException
	{
		...
		// we use a synchronized block rather than use a synchronized method so
		// that we
		...
		synchronized (this)
		{
			...
			// the Folder field. We need to make sure our mailbox is selected
			// while
			// we're working things.
			IMAPConnection currentConnection = ((IMAPStore) store).getFolderConnection(this);
			...
			try
			{
				...
				IMAPMailboxStatus status = currentConnection
						.openMailbox(fullname, mode == Folder.READ_ONLY);
				...
				if (status.mode != mode)
				{
					...
					if (mode == READ_WRITE)
					{
						throw new ReadOnlyFolderException(this,
															"Cannot open READ_ONLY folder in READ_WRITE mode");
					}
				}
				...
				// now copy the status information over and flip over the open
				// sign.
	...
	/**
	 * Populate the message cache with dummy messages, ensuring we're filled up
	 * to the server-reported number of messages.
	 * 
	 * @exception MessagingException
	 */
	...
	/**
	 * Close this folder; it must already be open. A @link
	 * ConnectionEvent#CLOSED} event is sent to all listeners registered {* with
	 * this folder.
	 * 
	 * @param expunge
	 *            whether to expunge all deleted messages
	 * @throws MessagingException
	 *             if there was a problem accessing the store; the folder is
	 *             still closed
	 */
	public synchronized void close(boolean expunge) throws MessagingException
	{
		...
		checkOpen();
		cleanupFolder(expunge, false, null);
	}

	/**
	 * Do folder cleanup. This is used both for normal close operations, and
	 * adnormal closes where the server has sent us a BYE message.
	 * 
	 * @param expunge
	 *            Indicates whether open messages should be expunged.
	 * @param disconnected
	 *            The disconnected flag. If true, the server has cut us off,
	 *            which means our connection can not be returned to the
	 *            connection pool.
	 * 
	 * @exception MessagingException
	 */
	protected synchronized void cleanupFolder(boolean expunge, boolean disconnected,
			IMAPConnection currentConnection) throws MessagingException
	{
		folderOpen = false;
		seqToUid = new HashMap<Integer, Long>();
		uidToSeq = new HashMap<Long, Integer>();
		...
		if (currentConnection != null)
		{
			...
			{
				// The CLOSE operation depends on what mode was used to select
				// the mailbox.
				// If we're open in READ-WRITE mode, we used a SELECT operation.
				// When CLOSE
				// is issued, any deleted messages will be expunged. If we've
				// been asked not
				// to expunge the messages, we have a problem. The solution is
				// to reselect the
				// mailbox using EXAMINE, which will not expunge messages when
				// closed.
	...
	/**
	 * Return the number of messages this folder contains. If this operation is
	 * invoked on a closed folder, the implementation may choose to return -1 to
	 * avoid the expense of opening the folder.
	 * 
	...
	public synchronized int getMessageCount() throws MessagingException
	{
		...
		// if we haven't opened the folder yet, we might not have good status
		// information.
	...
	/**
	 * Return the numbew of messages in this folder that have the
	 * {@link Flag.RECENT} flag set. If this operation is invoked on a closed
	 * folder, the implementation may choose to return -1 to avoid the expense
	 * of opening the folder. The default implmentation of this method iterates
	 * over all messages in the folder; subclasses should override if possible
	...
	/**
	 * Return the number of messages in this folder that do not have the
	 * {@link Flag.SEEN} flag set. If this operation is invoked on a closed
	 * folder, the implementation may choose to return -1 to avoid the expense
	 * of opening the folder. The default implmentation of this method iterates
	 * over all messages in the folder; subclasses should override if possible
	 * to provide a more efficient implementation.
	...
	public synchronized int getUnreadMessageCount() throws MessagingException
	{
		...
		{
			// if we have an open connection, then search the folder for any
			// messages
			// marked UNSEEN.
			...
			try
			{
				// search using the connection directly rather than calling our
				// search() method so we don't
				// need to instantiate each of the matched messages. We're
				// really only interested in the count
				// right now.
				long[] matches = connection.searchMailbox(criteria);
	...
	/**
	 * Return the number of messages in this folder that have the
	 * {@link Flag.DELETED} flag set. If this operation is invoked on a closed
	 * folder, the implementation may choose to return -1 to avoid the expense
	 * of opening the folder. The default implmentation of this method iterates
	 * over all messages in the folder; subclasses should override if possible
	 * to provide a more efficient implementation.
	...
	public synchronized int getDeletedMessageCount() throws MessagingException
	{
		...
		if (!folderOpen)
		{
			// the status update doesn't return deleted messages. These can only
			// be obtained by
	...
	/**
	 * Retrieve the message with the specified index in this Folder; messages
	 * indices start at 1 not zero. Clients should note that the index for a
	 * specific message may change if the folder is expunged; {@link Message}
	 * objects should be used as references instead.
	 * 
	 * @param msgNum
	 *            The message sequence number of the target message.
	 * 
	 * @return the message
	 * @throws MessagingException
	 *             if there was a problem accessing the store
	...
	public synchronized Message getMessage(int msgNum) throws MessagingException
	{
		// Can only be performed on an Open folder
		checkOpen();
		// Check the validity of the message number. This may require pinging
		// the server to
		...
		if (uid <= 0) // invalid UID
		{
			return null;
		}
		IMAPMessage msg = uidToMsg.get(uid);
		if (msg == null)
		{
			msg = new IMAPMessage(this, ((IMAPStore) store), msgNum, msgNum);
			msg.setUID(uid);
			uidToMsg.put(uid, msg);
		}
		return msg;
	}

	public synchronized Message getMessage(long uid, int msgNum) throws MessagingException
	{
		// Can only be performed on an Open folder
		checkOpen();
		// Check the validity of the message number. This may require pinging
		...
		if (msg == null)
		{
			...
			msg.setUID(uid);
			uidToMsg.put(uid, msg);
			seqToUid.put(msgNum, uid);
			uidToSeq.put(uid, msgNum);
		}
		return msg;
	}

	public synchronized long getMessageUid(int seqNum) throws MessagingException
	{
		Long l = seqToUid.get(seqNum);
		if (l == null)
		{
			IMAPConnection connection = getConnection();
			try
			{
				IMAPUid uid = connection.getUidForSequenceNumber(seqNum);
				l = uid.uid;
			}
			finally
			{
			...
			seqToUid.put(seqNum, l);
			uidToSeq.put(l, seqNum);

		}
		return l;
	}

	public synchronized int getSequenceNum(long uid) throws MessagingException
	{
		Integer seq = uidToSeq.get(uid);
		if (seq == null)
		{
			IMAPConnection connection = getConnection();
			try
			{
				IMAPUid id = connection.getSequenceNumberForUid(uid);
				seq = id.messageNumber;
			}
			finally
			{
				releaseConnection(connection);
			}
			seqToUid.put(seq, uid);
			uidToSeq.put(uid, seq);
		}
		return seq;
	}

	/**
	 * Retrieve a range of messages for this folder. messages indices start at 1
	 * not zero.
	 * 
	 * @param start
	 *            Index of the first message to fetch, inclusive.
	 * @param end
	 *            Index of the last message to fetch, inclusive.
	 * 
	...
	/**
	 * Append the supplied messages to this folder. A {@link MessageCountEvent}
	 * is sent to all listeners registered with this folder when all messages
	 * have been appended. If the array contains a previously expunged message,
	 * it must be re-appended to the Store and implementations must not abort
	 * this operation.
	 * 
	 * @param msgs
	 ...
	 * implementation; the default implementation in this class simply returns.
	 * 
	 * @param messages
	 *            messages for which information should be fetched
	 * @param profile
	...
	public void fetch(Message[] messages, FetchProfile profile) throws MessagingException
	{
		...
		// we might already have the information being requested, so ask each of
		// the
		// messages in the list to evaluate itself against the profile. We'll
		// only ask
		...
		for (int i = 0; i < messages.length; i++)
		{
			...
			// the message is missing some of the information still. Keep this
			// in the list.
			// even if the message is only missing one piece of information, we
			// still fetch everything.
		...
		try
		{
			// ok, from this point onward, we don't want any threads messing
			// with the
			// message cache. A single processed EXPUNGE could make for a very
			// bad day
			synchronized (this)
			{
				...
				// IMPORTANT: We must do our updates while synchronized to keep
				// the
				// cache from getting updated underneath us. This includes
				// not releasing the connection until we're done to delay
				// processing any
	...
	/**
	 * Set flags on the messages to the supplied value; all messages must belong
	 * to this folder. This method may be overridden by subclasses that can
	 * optimize the setting of flags on multiple messages at once; the default
	 * implementation simply calls {@link Message#setFlags(Flags, boolean)} for
	 * each supplied messages.
	 * 
	 * @param messages
	 *            whose flags should be set
	 * @param flags
	 *            the set of flags to modify
	 * @param set
	 *            Indicates whether the flags should be set or cleared.
	 * 
	 * @throws MessagingException
	 *             if there was a problem accessing the store
	 */
	public void setFlags(Message[] messages, Flags flags, boolean set) throws MessagingException
	{
		...
		synchronized (this)
		{
			...
			if (!availableFlags.contains(flags))
			{
				throw new MessagingException(
												"The IMAP server does not support changing of this flag set");
			}
			...
			try
			{
				...
				// retrieve each of the messages from our cache, and do the flag
				// update.
				// we need to keep the list so we can broadcast a change update
				// event
				// when we're finished.
				for (int i = 0; i < responses.size(); i++)
				{
					...
					// this shouldn't happen, but it might have been expunged
					// too.
					if (message != null)
					{
		...
		// ok, we're no longer holding the lock. Now go broadcast the update for
		// each
	...
	/**
	 * Set flags on a range of messages to the supplied value. This method may
	 * be overridden by subclasses that can optimize the setting of flags on
	 * multiple messages at once; the default implementation simply gets each
	 * message and then calls {@link Message#setFlags(Flags, boolean)}.
	 * 
	 * @param start
	 *            first message end set
	 * @param end
	 *            last message end set
	 * @param flags
	 *            the set of flags end modify
	 * @param value
	 *            Indicates whether the flags should be set or cleared.
	 * 
	 * @throws MessagingException
	 *             if there was a problem accessing the store
	 */
	public synchronized void setFlags(int start, int end, Flags flags, boolean value)
			throws MessagingException
	{
	...
	/**
	 * Set flags on a set of messages to the supplied value. This method may be
	 * overridden by subclasses that can optimize the setting of flags on
	 * multiple messages at once; the default implementation simply gets each
	 * message and then calls {@link Message#setFlags(Flags, boolean)}.
	 * 
	 * @param ids
	 *            the indexes of the messages to set
	 * @param flags
	 *            the set of flags end modify
	 * @param value
	 *            Indicates whether the flags should be set or cleared.
	 ...
	 *             if there was a problem accessing the store
	 */
	public synchronized void setFlags(int ids[], Flags flags, boolean value)
			throws MessagingException
	{
	...
	/**
	 * Copy the specified messages to another folder. The default implementation
	 * simply appends the supplied messages to the target folder using
	 * {@link #appendMessages(Message[])}.
	 * 
	 * @param messages
	...
	public synchronized void copyMessages(Message[] messages, Folder folder)
			throws MessagingException
	{
		// the default implementation just appends the messages to the target.
		// If
		// we're copying between two folders of the same store, we can get the
		// server to
		// do most of the work for us without needing to fetch all of the
		// message data.
		// If we're dealing with two different Store instances, we need to do
		// this the
		// hardway.
		...
		try
		{
			// ask the server to copy this information over to the other
			// mailbox.
	...
	/**
	 * Permanently delete all supplied messages that have the DELETED flag set
	 * from the Store. The original message indices of all messages actually
	 * deleted are returned and a {@link MessageCountEvent} event is sent to all
	 * listeners with this folder. The expunge may cause the indices of all
	 * messaged that remain in the folder to change.
	 * 
	...
	public synchronized Message[] expunge() throws MessagingException
	{
		...
		try
		{
			// send the expunge notification. This operation results in
			// "nn EXPUNGE" responses getting returned
			// for each expunged messages. These will be dispatched to our
			// response handler, which will process
			// the expunge operation. We could process this directly, but we may
			// have received asynchronous
		...
		// we get one EXPUNGE message for each message that's expunged. They
		// MUST be processed in
		// order, as the message sequence numbers represent a relative position
		// that takes into account
		// previous expunge operations. For example, if message sequence numbers
		// 5, 6, and 7 are
		// expunged, we receive 3 expunge messages, all indicating that message
		// 5 has been expunged.
		...
		synchronized (this)
		{
			// expunge all of the messages from the message cache. This keeps
			// the sequence
			// numbers up to-date.
		...
		// if we have messages that have been removed, broadcast the
		// notification.
		...
		// note, we're expected to return an array in all cases, even if the
		// expunged count was zero.
		return messages;
	}

	/**
	 * Search the supplied messages for those that match the supplied criteria;
	 * messages must belong to this folder. The default implementation iterates
	 * through the messages, returning those whose
	 * {@link Message#match(javax.mail.search.SearchTerm)} method returns true;
	...
	public synchronized Message[] search(SearchTerm term) throws MessagingException
	{
		...
		try
		{
			...
			long[] messageNumbers = connection.searchMailbox(term);
	 ...
	 *             if there was a problem accessing the store
	 */
	public synchronized Message[] search(SearchTerm term, Message[] messages)
			throws MessagingException
	{
		// only allowed on open folders
		checkOpen();
		...
		// the expunged messages first.
		String messageSet = generateMessageSet(messages);

		// If we have no "live" messages to search, just return now. We're
		// required to return a non-null
		...
		try
		{
			...
			long[] messageNumbers = connection.searchMailbox(messageSet, term);
			return resolveMessages(messageNumbers);
		}
		finally
		{
	...
	public synchronized long[] searchUidBySequenceRange(int lowSeq, int highSeq)
			throws MessagingException
	{
		// only allowed on open folders
		checkOpen();
		...
		try
		{
			...
			// now go do the search.
			// return connection.rawSearch("UID SEARCH UID",
			// lowUid+":"+highUid);
			IMAPSearchResponse rsp = connection.rawSearch("UID SEARCH", lowSeq + ":" + highSeq);
			long[] uids = rsp.messageNumbers;
			assert highSeq - lowSeq + 1 == (long) uids.length;

			for (int i = 0; i < uids.length; i++)
			{
				seqToUid.put(lowSeq + i, uids[i]);
				uidToSeq.put(uids[i], lowSeq + i);
			}
			return rsp.messageNumbers;
		}
		finally
		{
			releaseConnection(connection);
	 ...
	 *            The target UID.
	 * 
	 * @return the Message object. Returns null if the message does not exist.
	 * @exception MessagingException
	 */
	public synchronized Message getMessageByUID(long uid) throws MessagingException
	{
		...
		if (msg == null)
		{
			int seqNum = getSequenceNum(uid);
			msg = new IMAPMessage(this, ((IMAPStore) store), seqNum, seqNum);
			msg.setUID(uid);
			uidToMsg.put(uid, msg);
		}
		return msg;
	...
	/**
	 * Get a series of messages using a UID range. The special value LASTUID can
	 * be used to mark the last available message.
	 * 
	 * @param start
	 *            The start of the UID range.
	 * @param end
	 *            The end of the UID range. The special value LASTUID can be
	 *            used to request all messages up to the last UID.
	 * 
	 * @return An array containing all of the messages in the range.
	 * @exception MessagingException
	 */
	public synchronized Message[] getMessagesByUID(long start, long end) throws MessagingException
	{
		...
		try
		{
			...
			List<IMAPUid> uids = connection.getSequenceNumbersForUids(start, end);
			Message[] msgs = new Message[uids.size()];

			// fill in each of the messages based on the returned value
			ListIterator<IMAPUid> itr = uids.listIterator();
			int i = 0;
			while (itr.hasNext())
			{
				IMAPUid uid = itr.next();
				msgs[i] = getMessage(new Long(uid.uid), uid.messageNumber);
				++i;
			}
			/*
			 * for (int i = 0; i < msgs.length; i++) { IMAPUid uid =
			 * (IMAPUid)uids.get(i); msgs[i] = getMessage(new Long(uid.uid),
			 * uid.messageNumber); }
			 */
	...
	/**
	 * Retrieve a set of messages by explicit UIDs. If any message in the list
	 * does not exist, null will be returned for the corresponding item.
	 * 
	 * @param ids
	 *            An array of UID values to be retrieved.
	 * 
	 * @return An array of Message items the same size as the ids argument
	 *         array. This array will contain null entries for any UIDs that do
	 *         not exist.
	 * @exception MessagingException
	 */
	...
	/**
	 * Retrieve the UID for a message from this Folder. The argument Message
	 * MUST belong to this Folder instance, otherwise a NoSuchElementException
	 * will be thrown.
	 * 
	 * @param message
	 *            The target message.
	...
	protected synchronized void appendMessage(Message msg) throws MessagingException
	{
		...
		// convert the message into an array of bytes we can attach as a
		// literal.
	 ...
	 */
	protected synchronized Folder[] filterFolders(String pattern, boolean subscribed)
			throws MessagingException
	{
		IMAPConnection connection = getConnection();
		...
		if(pattern == null)
			pattern="%";
		String root=fullname==""?"":fullname + getSeparator();
		...
		for (int i = 0; i < responses.size(); i++)
		{
			...
			// if a full wildcard is specified, the root folder can be returned
			// too. Make sure we
			// filter that one out.
			if (!response.mailboxName.equals(root))
			{
				IMAPFolder folder = new IMAPFolder((IMAPStore) store, response);
				folders.add(folder);
			}
		}
	...
	/**
	 * Validate that a target message number is considered valid by the IMAP
	 * server. If outside of the range we currently are a ware of, we'll ping
	 * the IMAP server to see if there have been any updates.
	 * 
	 * @param messageNumber
	 ...
	 * @exception MessagingException
	 */
	protected synchronized void checkMessageValidity(int messageNumber) throws MessagingException
	{
		...
		synchronized (this)
		{
			try
			{
				// ping the server to see if there's any updates to process. The
				// updates are handled
	...
	// Tim liu add at 11/19/2011 for sending simple command such as "noop"
	// "idle" to keep alive with server before
	// starting to receive mails.
	public void sendSimpleCommand(String cmd)
	{
		try
		{
			IMAPConnection currentConnection = getConnection();
			synchronized (this)
			{
				try
				{
					// ping the server to see if there's any updates to process.
					// The updates are handled
					// by the response handlers.
					currentConnection.sendSimpleCommand(cmd);
				}
				finally
				{
					releaseConnection(currentConnection);
	...
	/**
	 * Ensure the folder is open. Throws a MessagingException if not in the
	...
	/**
	 * Ensure the folder is not open for operations that require the folder to
	 * be closed.
	 * 
	 * @exception IllegalStateException
	 */
	...
	/**
	 * Ensure that the folder is open for read/write mode before doing an
	 * operation that would make a change.
	 * 
	 * @exception IllegalStateException
	 */
	...
	/**
	 * Notify the message changed listeners that a message contained in the
	 * folder has been updated.
	 * 
	 * @param type
	 *            The type of update made to the message.
	 * @param m
	...
	/**
	 * Retrieve the connection attached to this folder. Throws an exception if
	 * we don't have an active connection.
	 * 
	...
	protected synchronized IMAPConnection getConnection() throws MessagingException
	{
		...
		if (fullname != null && fullname.length() > 0
			&& !fullname.equals(connection.currentFolderName))
		{
			IMAPMailboxStatus status = connection.openMailbox(fullname, mode == Folder.READ_ONLY);
		}
		// we need to make ourselves a handler of unsolicited responses
	 ...
	 * @exception MessagingException
	 */
	protected synchronized void releaseConnection(IMAPConnection connection) throws MessagingException
	{
		...
		// finished processing the expected responses. We need to do this
		// because
		// the unsolicited responses may include EXPUNGED messages. The EXPUNGED
		// messages will alter the message sequence numbers for the messages in
		// the
		// cache. Processing the EXPUNGED messages too early will result in
		// updates getting applied to the wrong message instances. So, as a
		// result,
		// we delay that stage of the processing until all expected responses
		// have
		// been handled.
		// process any pending messages before returning.
		
		//Daniel Liu, 2012.05.22, move release first, so this connection can be reused during process pending response
		((IMAPStore) store).releaseFolderConnection(this, connection);
		connection.processPendingResponses();
		// if no cached connection or this is somehow different from the
		// cached one, just
		// return it.
			connection.removeResponseHandler(this);
			connection.setIdleHandler(null);
			
			//LiuLele, 2013.03.06 releaseConnection was found called recursively, so don't set it to null to avoid NullPoint
//			if(currentConnection == connection)
//				currentConnection = null;
		// if we're open, then we don't have to worry about returning this
		// connection
	 ...
	 * will be the Folder's connection, which is only available if the Folder is
	 * currently open.
	 * 
	...
	{
		IMAPConnection connection = ((IMAPStore) store).getFolderConnection(this);
		if (!fullname.equals(connection.currentFolderName))
		{
			IMAPMailboxStatus status = connection.openMailbox(fullname, mode == Folder.READ_ONLY);
		}
		return connection;
	...
	{
		// release it back to ourselves...this will drive unsolicited message
		// processing.
		releaseConnection(connection);
	}
	 ...
	 * 
	 * @param force
	 *            Force a status refresh always.
	 * 
	 * @exception MessagingException
	 */
	public synchronized void refreshStatus(boolean force) throws MessagingException
	{
		// first check that any cached status we've received has gotten a little
		// moldy.
		...
		uidNext = cachedStatus.uidNext;
		populateMessageCache();
	}

	/**
	 * Process an EXPUNGE response for a message, removing the message from the
	 * message cache.
	 * 
	 * @param sequenceNumber
	 *            The sequence number for the expunged message.
	 * 
	 * @return The Message object corresponding to this expunged message.
	 * @exception MessagingException
	 */
	protected synchronized Message expungeMessage(int sequenceNumber) throws MessagingException
	{
		// we need to remove this from our message list. Deletion of a message
		// by
		// sequence number shifts the sequence numbers of every message after
		// the target. So we, need to remove the message, update its status,
		// then
		// update the sequence numbers of every message after that. This is a
		// serious
		// pain!
		Long uid = seqToUid.get(sequenceNumber);
		if (uid == null)
			return null;
		Message msg = uidToMsg.get(uid);
		if (msg != null)
		{
			uidToMsg.remove(uid);
		}
		seqToUid.remove(sequenceNumber);
		uidToSeq.remove(uid);
		maxSequenceNumber--;
		return msg;
	}
	...
	/**
	 * Resolve an array of message numbers into an array of the referenced
	 * messages.
	 * 
	 * @param messageNumbers
	 *            The array of message numbers (can be null).
	 * 
	 * @return An array of Message[] containing the resolved messages from the
	 *         list. Returns a zero-length array if there are no messages to
	 *         resolve.
	 * @exception MessagingException
	 */
	protected Message[] resolveMessages(long[] messageNumbers) throws MessagingException
	{
		// the connection search returns a null pointer if nothing was found,
		// just convert this into a
		// null array.
		...
		{
			messages[i] = getMessage((int) messageNumbers[i]);
		}
	...
	/**
	 * Take an array of messages and generate a String <message set> argument as
	 * specified by RFC 2060. The message set argument is a comma-separated list
	 * of message number ranges. A single element range is just one number. A
	 * longer range is a pair of numbers separated by a ":". The generated
	 * string should not have any blanks. This will attempt to locate
	 * consequetive ranges of message numbers, but will only do this for
	 * messages that are already ordered in the array (i.e., we don't try to
	 * sort). Expunged messages are excluded from the search, since they don't
	 * exist anymore. A valid search string will look something like this:
	 * 
	 * "3,6:10,15,21:35"
	 * 
	 * @param messages
	 *            The array of messages we generate from.
	 * 
	 * @return A string formatted version of these message identifiers that can
	 *         be used on an IMAP command.
	 */
	protected String generateMessageSet(Message[] messages) throws MessagingException
	{
		...
		for (int i = 0; i < messages.length; i++)
		{
			...
			if (!start.isExpunged())
			{
				...
				// we can go ahead and add this to the list now. If we find this
				// is the start of a
				// range, we'll tack on the ":end" bit once we find the last
				// message in the range.
				if (set.length() != 0)
				{
					// only append the comma if not the first element of the
					// list
					set.append(',');
				}

				// append the first number. NOTE: We append this directly rather
				// than
				// use appendInteger(), which appends it using atom rules.
				set.append(Integer.toString(start.getSequenceNumber()));

				// ok, we have a live one. Now scan the list from here looking
				// for the end of
				...
				// have a range end point? Add the range specifier and step the
				// loop index point
				// to skip over this
				if (endIndex != -1)
				{
					...
					set.append(':');
					set.append(Integer.toString(((IMAPMessage) messages[endIndex])
							.getSequenceNumber()));
				}
			}
		}

		// return null for an empty list. This is possible because either an
		// empty array has been handed to
	...
	/**
	 * Verify that this folder exists on the server before performning an
	 * operation that requires a valid Folder instance.
	 * 
	 * @exception MessagingException
	 */
	...
	/**
	 * Search a list of LIST responses for one containing information for a
	 * particular mailbox name.
	 * 
	 * @param responses
	...
	protected IMAPListResponse findListResponse(List responses, String name)
	{
		...
		{
			IMAPListResponse response = (IMAPListResponse) responses.get(i);
			if (response.mailboxName.equalsIgnoreCase(name))
			{
				return response;
			}
		}
	...
	/**
	 * Protected class intended for subclass overrides. For normal folders, the
	 * mailbox name is fullname. For Namespace root folders, the mailbox name is
	 * the prefix + separator.
	 * 
	 * @return The string name to use as the mailbox name for exists() and
	 *         issubscribed() calls.
	 */
	...
	/**
	 * Handle an unsolicited response from the server. Most unsolicited
	 * responses are replies to specific commands sent to the server. The
	 * remainder must be handled by the Store or the Folder using the
	 * connection. These are critical to handle, as events such as expunged
	 * messages will alter the sequence numbers of the live messages. We need to
	 * keep things in sync.
	 * 
	 * @param response
	 *            The UntaggedResponse to process.
	 * 
	 * @return true if we handled this response and no further handling is
	 *         required. false means this one wasn't one of ours.
	 */
	public synchronized boolean handleResponse(IMAPUntaggedResponse response)
	{
		...
		if (response.isKeyword("EXISTS"))
		{
			...
			// we need to update our cache, and also retrieve the new messages
			// and
			...
			// has the size grown? We have to send the "you've got mail"
			// announcement.
			if (oldCount < maxSequenceNumber)
			{
				try
				...
				{
					//may catch java.lang.IllegalStateException: Folder is not Open, when called from IMAPFolder.sendIdle
					/*
					 java.lang.IllegalStateException: Folder is not Open
     at org.apache.geronimo.javamail.store.imap.IMAPFolder.checkOpen(IMAPFolder.java:2089)
     at org.apache.geronimo.javamail.store.imap.IMAPFolder.getMessages(IMAPFolder.java:1213)
     at org.apache.geronimo.javamail.store.imap.IMAPFolder.handleResponse(IMAPFolder.java:2606)
     at org.apache.geronimo.javamail.store.imap.connection.IMAPConnection.processPendingResponses(IMAPConnection.java:737)
     at org.apache.geronimo.javamail.store.imap.IMAPFolder.releaseConnection(IMAPFolder.java:2211)
     at org.apache.geronimo.javamail.store.imap.IMAPFolder.sendIdle(IMAPFolder.java:2973)
     at fortunedog.mail.proxy.ImapProtocol.waitingNewMail(ImapProtocol.java:851)
     at fortunedog.mail.proxy.MailClient.waitingNewMail(MailClient.java:1133)
     at fortunedog.mail.proxy.checker.MailChecker.run(MailChecker.java:54)
					 */
		...
		// this is a BYE response on our connection. This forces us to close,
		// but
		...
		else if (response.isKeyword("BYE"))
		{
			...
			try
			{
				cleanupFolder(false, true, response.getConnection());
			}
			catch (MessagingException e)
			{
			}
		...
		else if(response.isKeyword("UIDNEXT"))
		{
			this.uidNext = ((IMAPSizeResponse) response).getSize();
		}
	...
	// The following set of methods are extensions that exist in the Sun
	// implementation. They
	// match the Sun version in intent, but are not 100% compatible because the
	// Sun implementation
	// uses com.sun.* class instances as opposed to the org.apache.geronimo.*
	// classes.
	 ...
	 * 
	 * @param acl
	 *            The ACL describing the Rights to remove.
	 * 
	 * @exception MessagingException
	 */
	 ...
	 * 
	 * @param quota
	 *            The new quota information to set.
	 * 
	 * @exception MessagingException
	 */
	...
	/**
	 * Get the set of attributes defined for the folder as the set of
	 * capabilities returned when the folder was opened.
	 * 
	...
	public synchronized void addToUidCache(long uid, IMAPMessage imapMessage)
			throws MessagingException
	{
		uidToSeq.remove(uid);
		uidToMsg.put(uid, imapMessage);

	}

	public boolean hasChild()
	{
		return listInfo.hasChild;
	}
	
	public boolean isNoSelectable()
	{
		return listInfo.noselect;
	}
	...
	public boolean hasCapability(String capability) throws MessagingException
	{
		...
		{
			conn = getConnection();
			return conn.hasCapability(capability);
		}
		finally
		{
	...
	private boolean hasDoneSend = false;
	private Semaphore idleLock = new Semaphore(1); //to make sure DONE will not be send twice. Must use semaphore but not synchronized(x), because
													//we need to release it during sub-function call.
	
	public void sendIdle() throws MessagingException, InterruptedException
	{
		...
		try 
		{
			currentConnection.setIdleHandler(new IMAPUntaggedResponseHandler() {
				public boolean handleResponse(IMAPUntaggedResponse response)
				{
					if (response.isKeyword("EXISTS"))
					{
						try {
							//log.debug("EXISTS returned, to acquire lock, available permits:{}", idleLock.availablePermits());
							idleLock.acquire();
							//log.debug("EXISTS returned,lock acquired, available permits:{}", idleLock.availablePermits());
						...
						try
						{
							...
							if(hasDoneSend)
								return false;
							log.debug( "Server return EXISTS, Send DONE to complete IDLE");
							currentConnection.write("DONE\r\n");
							hasDoneSend = true;
						}
						catch (MessagingException e)
						{
							log.error( "Fail to send DONE", e);
						}
						finally
						{
							
							idleLock.release();
							//log.debug("EXISTS processed, lock released. :{}", idleLock.availablePermits());
						}
					}
					return false;
				}
			});
			idleLock.acquire(); //released in IdleHook
			log.debug( "Send IDLE...");
			hasDoneSend = false;
			currentConnection.sendSimpleCommand("IDLE", new IdleHook() {
				
				public void run() {
					//log.debug("IDLE sent, to release lock, {}", idleLock.availablePermits());
					idleLock.release();
					//log.debug("IDLE sent, lock released, {}", idleLock.availablePermits());
	...
	public void  interruptIdle() throws MessagingException, InterruptedException
	{
		//log.debug("interruptIdle, to acquire lock, available permits:{}", idleLock.availablePermits());
		idleLock.acquire();
		//log.debug("interruptIdle, lock acquired, available permits:{}", idleLock.availablePermits());
		try
		{
			if(hasDoneSend)
				return;
			//log.debug( "call in interruptIdle");
			
			if(currentConnection != null && currentConnection.isSocketAlive())
			{
				log.debug( "Send DONE to terminate IDLE");
				currentConnection.write("DONE\r\n");
				hasDoneSend = true;
			}
			else
				log.debug("not send anything, currentConnection is null");
		}
		finally
		{
			idleLock.release();
			//log.debug("interruptIdle finihed, lock released");
		}
	}
