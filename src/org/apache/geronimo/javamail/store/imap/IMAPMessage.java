...
public class IMAPMessage extends MimeMessage {
   ...
   // protected int sequenceNumber;
    ...
    public void setExpunged(boolean value) {
        ...
        if (isExpunged()) {
            IMAPFolder f = (IMAPFolder) getFolder();
            try
			{
				f.expungeMessage(getSequenceNumber());
			}
    ...
    public synchronized void setFlags(Flags flag, boolean set) throws MessagingException {
        ...
        synchronized (folder) {
            ...
            try {
                ...
                // server. 
                flags = connection.setFlags(getSequenceNumber(), flag, set); 
            } finally {
     ...
     * @return if has attachment then return true.
     */
    public boolean hasAttachment() throws MessagingException
    {
    	loadBodyStructure();
    	return bodyStructure.hasAttachment();
    }
    /**
     * Tim Liu add this function at 05/28/2012
     * @return attachment'f file name list, if there's no attachment,empty list will be returned.
     */
    public LinkedList<String> getAttachmentNames() throws MessagingException
    {
    	loadBodyStructure();
    	return bodyStructure.getAttachmentNames();
    }
    ...
    protected void updateHeader(String header, InternetAddress[] addresses) throws MessagingException {
        if (addresses != null) {
        	Vector<InternetAddress> v = new Vector<InternetAddress>(addresses.length);
        	...
        	if(v.size() > 0)
        	{
        		if(v.size() != addresses.length)
        		{
        			addresses = new InternetAddress[v.size()];
        			v.toArray(addresses);
        		}
    ...
    public void loadFlags() throws MessagingException {
        ...
        synchronized (folder) {
            ...
            try {
                ...
                flags = connection.fetchFlags(getSequenceNumber()); 
            } finally {
                releaseConnection(connection); 
            }
        }
    ...
    protected synchronized void loadHeaders() throws MessagingException {
        ...
        synchronized (folder) {
            ...
            try {
                ...
                headers = connection.fetchHeaders(getSequenceNumber(), section);
     ...
     * @exception MessagingException
     */                                
    public synchronized void loadEnvelope() throws MessagingException {
        // don't retrieve if already loaded.
        if (envelope != null) {
            return;
        }
        ...
        synchronized (folder) {
            ...
            try {
                ...
                List fetches = connection.fetchEnvelope(getSequenceNumber());
    ...
    protected synchronized void loadBodyStructure() throws MessagingException {
        ...
        // used. 
        synchronized (folder) {
        	int seq = getSequenceNumber(); //call getSequenceNumber here, to avoid create nested connections
            IMAPConnection connection = getConnection();
            try {
                // fetch the envelope information for this
                bodyStructure = connection.fetchBodyStructure(seq);
    ...
    protected void loadContent() throws MessagingException {
        ...
        // used. 
        synchronized (folder) {
        	int seqnum = getSequenceNumber();//May.4, 2013, get sequence number first, don't nest call in calling fetchEntireMessage, to avoid duplicated connection creation
            IMAPConnection connection = getConnection();
            try {
                // load the content from the server. 
            	List<IMAPFetchDataItem> rst = connection.fetchEntireMessage(seqnum);
                for(IMAPFetchDataItem r : rst)
                {
                	if(r.isType(IMAPFetchDataItem.HEADER))
                	{
                        headers = ((IMAPInternetHeader)r).headers;
                        allHeadersRetrieved = true;                                               
                	}
                	else if(r.isType(IMAPFetchDataItem.TEXT))
                	{
                		content = ((IMAPMessageText)r).getContent();
                	}
                }
            } finally {
                releaseConnection(connection); 
     ...
     */
    int getSequenceNumber() throws MessagingException {
    	IMAPFolder f = (IMAPFolder) getFolder();
        return f.getSequenceNum(uid);
    }
     ...
     */
    synchronized void setSequenceNumber(int s) {
        IMAPFolder f = (IMAPFolder) getFolder();
        int oldSeq = f.uidToSeq.get(uid);
        f.seqToUid.remove(oldSeq);
        f.seqToUid.put(s, uid);
        f.uidToSeq.put(uid, s);
    }
